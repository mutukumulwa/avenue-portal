...
| **F5.1** | `claim-intake-direct-entry.integration.test.ts` | **PASS (M5 begins)** — admin (ADMIN_PORTAL) + provider (PROVIDER_PORTAL) submissions of the same claim normalize identically (same billed/lines/date, both MANUAL); accepted claim processed in-request (terminal `processingState`, not stuck PENDING); draft-UUID double-click ⇒ one receipt + one claim (replay); provider spoof (providerUser A names provider B) ⇒ rejected, no claim (D12); inpatient-no-PA ⇒ ACCEPTED + routed, NOT approved, `approvedAmount` 0 (D6, no money); future service date ⇒ rejected at the door. |
| **F5.2** | `claim-intake-api.integration.test.ts` | **PASS** — REAL minted `ProviderApiKey` auth end-to-end: 201 accept (receipt channel API_V1 + scope `provider:<id>`, source HMS not hardcoded SMART, `externalRef` stored, processed in-request); same-key replay = timeout-after-commit recovery (one claim); changed-payload same key ⇒ 409 `IDEMPOTENCY_KEY_REUSED`; legacy externalRef resend WITHOUT header ⇒ 200 original (cross-boundary continuity); un-entitled facility ⇒ non-enumerating 403 + facility spoof ⇒ 403 with zero claims (D12; real ProviderContract+ContractApplicability seeded); APPROVED PA attached atomically via `origin` (PA → ATTACHED); 20-way identical ⇒ exactly ONE claim; 20-way distinct ⇒ every 201 = one claim, no duplicate numbers, failures only clean retryable 503s (receipt PROCESSING, no claim). Suite re-runnable: two consecutive full sequential passes green (fixture pools now select only untouched claims). |

**⚠️ Entitlement finding (F5.1):** the disposable seed has **0** active provider contracts / `ContractApplicability` rows (6 providers, 247 members). The canonical context originally scoped members by provider entitlement on every provider-derived rail — which would have blocked EVERY provider-portal claim on such a tenant. Fixed by decoupling `scopeMembersByEntitlement` from `providerDerived`: the portal keeps its D12 spoof guard but resolves members tenant-wide (no regression); programmatic B2B/offline facility rails keep entitlement scoping. Any future rail must choose its member-scoping policy explicitly.
