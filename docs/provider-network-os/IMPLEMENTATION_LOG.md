...
## F9.9 — Cut over legacy HMS configuration/path (BUILT · flip GATED) ★ closes phase F9's buildable scope

```text
Work package: F9.9 (phase F9 — HMS integration control plane)
Status: BUILT · GATED. The reviewed-mapping + shadow-comparison + rollback-safe cutover-mode decision are built and tested. The live /api/v1/hms-batch route is DELIBERATELY UNTOUCHED — the actual traffic flip is a pilot-sign-off-gated ops step. No schema deletion, no legacy write path retired.
Commit: this feat commit (LegacyHmsCutoverService + test) + the paired docs commit (this note + PROGRESS row + phase note).
Proof-before-build classification: MISSING (no cutover support) but every dependency exists — the F9.3 admin (create), the F9.5 mapper + case matching, the legacy HmsBatchService (for the parity comparison), the legacy IntegrationConfig (the migration source). Files inspected: hms-batch.service.ts (the legacy apply + its case matching + ExceptionLog on unmatched), the F9.5 mapper, connection-admin.service (create). Smallest change: one service (map + shadow + decision); NO route change, NO schema. No CONFLICTING path.
Files changed: src/server/services/provider-integration/legacy-cutover.service.ts (new); tests/services/provider-integration-cutover.service.test.ts (new — 1 pure + 3 DB); docs PROGRESS + this note.
Schema/data changes: NONE.
Behavior delivered (the buildable steps): (1) reviewed-input mapping — mapConfigToConnection creates a DRAFT connection for the reviewer's provider from a legacy IntegrationConfig's endpoint (the tenant-level config has no provider/branch, so the reviewer's ctx supplies it; anti-widening is the F9.3 admin's); (2) dual/SHADOW comparison WITHOUT double mutation — shadowCompare is a READ-ONLY projection of what the new rail would do (map + match), proven to create ZERO CaseServiceEntry rows; (3) reconcile — its WOULD_APPLY count equals the legacy apply's applied count on the same batch (parity proven before any flip), and it surfaces a record the legacy validate would reject; (4) switch-behind-a-flag — resolveCutoverMode is the pure decision, DEFAULT LEGACY so an unset/false/unknown flag never diverts live traffic; (5) rollback-safe — flipping the flag back to unset/false restores LEGACY (proven by the default); (6) retire legacy write path = NOT done (gated; requires all connections cut over + sign-off).
Authorization evidence: mapConfigToConnection routes through the F9.3 admin (requirePermission(provider.integrations.manage) + provider is ctx.providerId + branch held). shadowCompare is a read-only projection (no external actor mutation).
Idempotency/concurrency evidence: shadowCompare mutates nothing (idempotent by construction). The eventual flip reuses the F9.4/F9.5 idempotent rail; the legacy per-line hashref remains the legacy path's guard until retirement.
Privacy/security evidence: shadowCompare returns counts + safe per-record outcomes (WOULD_APPLY/UNMATCHED/REJECTED + a safe reason) — no clinical body. No live route change means no new exposure surface.
Money/reconciliation evidence: THE cutover safety proof — the new rail's projected apply count reconciles to the legacy apply count BEFORE any flip (parity), and the shadow makes zero financial/clinical mutation (proven: the case has zero entries after the shadow, one after the legacy apply). This is the "no duplicate apply + before/after reconciliation" evidence the package requires.
Focused tests and results: 1 pure (resolveCutoverMode defaults LEGACY for unset/false/unknown; SHADOW/CONNECTION/true map through) + 3 DB (shadow parity + zero-mutation vs the legacy apply on the same batch; shadow surfaces a legacy-reject without mutation; reviewed config→DRAFT connection with the endpoint + BIDIRECTIONAL mode). 4/4 pass; the pure block runs in the no-DB suite, the DB block self-skips without AUTOPILOT_TEST_DB. All 8 integration suites co-run 56/56. tsc + brand + currency green; full no-DB suite green (no regression).
Typecheck/schema result: tsc --noEmit clean; no schema change.
Manual/visual evidence: N/A — service only.
Feature-flag state: GATED — resolveCutoverMode exists but NO caller flips it; the live hms-batch route is unchanged (LEGACY). Enabling SHADOW then CONNECTION for one connection is the pilot-sign-off-gated ops sequence.
Backfill/rollout impact: none until the gated flip. The migration (mapConfigToConnection) is run per-provider with reviewed input at cutover time.
Known limitations / deferrals (flagged): (a) LIVE FLIP GATED — wiring resolveCutoverMode into the hms-batch route + the dual-run + the per-connection switch + the legacy-write-path retirement all need the pilot sign-off (not fabricated); (b) shadowCompare compares mapping+matching (the divergence risk) — both rails call the SAME CaseService at apply time, so apply-validation parity is inherent; (c) the route is intentionally untouched so the legacy path stays byte-identical until the gated cutover.
Unrelated worktree changes preserved: yes — worktree contained only scratchpad/ (untracked) + the F9.9 files; the main-checkout dirty UAT files untouched; no schema/route change.
Next allowed package: F10.1 — Approve capitation arrangement + accounting specification (M). GATED(finance/legal/network/client/provider). A governance artifact like F6.1/F7.1/F8.1: write the versioned worked-rules doc (eligible lives, rates, services, carve-outs, periods, adjustments, GL, invoice/voucher/payment, corrections) grounded in real schema/service fact fields; do NOT activate; surface for the six-owner sign-off (PENDING).
...
## F11.1–F11.4 + F11.7 — System-hardening suites + runbooks (buildable scope)

```text
Work packages: F11.1 (security matrix), F11.2 (concurrency/idempotency), F11.3 (money conservation), F11.4 (privacy/log/export scan), F11.7 (ops runbooks)
Status: COMPLETE for the buildable scope (F9/F10 surfaces + capitation ledger). F11.5 (load) + F11.6 (a11y) DEFERRED (need a production-like dataset + a real browser/deployment — the worktree cannot browser/HTTP-verify). F11.8–F11.10 are human/operational gates (actor UAT, pilot, GA) — not code.
Commit: this feat commit (openPeriod hardening + 4 automated suites) + the paired docs commit (SECURITY_MATRIX + OPERATIONS_RUNBOOKS + PROGRESS rows + this note).
Files changed: src/server/services/capitation/arrangement.service.ts (openPeriod now P2002-race-safe — a concurrency hardening surfaced by F11.2); tests/security/pnos-cross-boundary.test.ts (3 DB); tests/concurrency/repeatable-commands.test.ts (4 DB); tests/financial/capitation-conservation.test.ts (3 DB); tests/privacy/pnos-marker-scan.test.ts (2 DB); docs SECURITY_MATRIX.md + OPERATIONS_RUNBOOKS.md + PROGRESS + this note.
Schema/data changes: NONE.
Behavior delivered: (F11.1) a build-gating cross-boundary matrix proving cross-PROVIDER (A↛B), cross-TENANT (alpha↛beta), and ROLE denial across the integration + capitation surfaces, all non-enumerating, + SECURITY_MATRIX.md enumerating surface→enforcement→covering-test (F1–F8 via their per-package scope suites); (F11.2) REAL parallel (Promise.all against one DB) proofs of one-outcome-per-repeatable-command — a duplicate inbound delivery collapses to one row, a delivery lease has exactly one winner, a period opens once (after the openPeriod P2002 hardening), and accrual is value-stable under a concurrent recompute; (F11.3) the capitation conservation law (opening + gross + adj − paid = closing) holds Decimal-exact at EVERY transition of a full accrue→adjust→recalc→freeze→approve→pay→reverse→repay→close lifecycle, plus the zero-pay-capitated-never-FFS guard + overpay rejection; (F11.4) a marker scan — a live secret + a raw-body PHI marker + internal finance ids are seeded, then every ops read model / receipt / statement is scanned and none surfaces the markers or any structural secret/hash/lease handle; (F11.7) OPERATIONS_RUNBOOKS.md RB-1..8 (delivery stuck/poison, circuit, secret rotation, capitation mismatch/reversal, zero-pay-FFS, privacy incident) — each with symptoms, safe read-only queries, service-call actions, escalation, reconciliation, closure, and the no-manual-DB-edit rule.
Authorization/idempotency/privacy/money evidence: see each suite — cross-boundary denial (F11.1), one-logical-outcome under real concurrency (F11.2), exact-Decimal conservation + no-double-pay (F11.3), no-marker-leak (F11.4). One hardening applied: openPeriod is now concurrency-safe (P2002 → return the raced row).
Focused tests and results: 12 DB across the 4 suites, all pass on the throwaway PG; self-skip without AUTOPILOT_TEST_DB. New test dirs (tests/security|concurrency|financial|privacy) are collected by vitest. tsc + brand + currency green; full no-DB suite 1541 pass / 501 skip (no regression).
Typecheck/schema result: tsc clean; no schema change.
Feature-flag state: none — the suites are CI coverage; the runbooks are docs.
Known limitations / deferrals (flagged): (a) the suites focus on the F9/F10 surfaces I authored + the cross-tenant dimension — the F1–F8 surfaces rely on their existing per-package scope suites (referenced in SECURITY_MATRIX.md), NOT re-implemented here; (b) F11.5 load + F11.6 a11y need a production-like dataset + a real browser (a seeded deployment), so they are DEFERRED not done; (c) a full HTTP-request-level route matrix (guessed IDs at the /api/v1 + page layer) belongs with F11.8 actor UAT against a deployment; (d) F11.8–F11.10 (actor UAT, pilot, GA + legacy retirement) are human/operational gates.
Unrelated worktree changes preserved: yes — worktree contained only scratchpad/ (untracked) + the F11 files; the main-checkout dirty UAT files untouched.
Next allowed package: F11.5 (load) + F11.6 (a11y) against a seeded deployment; then the human gates F11.8 (actor UAT) → F11.9 (pilot activation, which is where the F9.7/F10.7 gated activations happen) → F11.10 (GA + legacy retirement).
Stop condition observed: yes — the buildable hardening suites + runbooks delivered; NO load/a11y (deferred to a deployment), NO human-gate execution.
```
