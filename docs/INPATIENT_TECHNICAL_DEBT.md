...
| A5 | **Full day-by-day scenario narratives (§12–17)** — ⏸ **VM-EXECUTION (not code-actionable)** | Each of the six scenarios' **key binding test** was exercised (maternity, Boda, stroke limit-exhaustion, malaria partial, burns package, foot readmission), but not the full per-day LOS / bed-day / per-line-version detail across the admission with Friday interim slices. | This is a **UAT execution pass on the controlled-clock Lima VM** (advance clock day-by-day, weekly `cutInterimSlice`, seven-ledger recon each Friday, final close) — nothing to build. Every mechanic it walks is already proven (interim slicing, recon, availability gate, PA read-through, and now concurrency A4). Stays open as a VM campaign; no local/code equivalent. | **P2 (VM)** |
