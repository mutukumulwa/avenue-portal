...
| A2 | **Reporting + GL / trial-balance tie-out (§24)** — 🟡 **AUDITED 2026-07-21: money PASS, 1 defect fixed** | The seven-ledger case reconciliation was proven at the case level and the GL nets to zero (OBS-IP-GL balanced 2026-07-19), but the **reports/exports** were not independently reconciled back to the ledgers. | **Money sums reconcile:** provider-/member-statements filter to APPROVED/PARTIALLY_APPROVED/PAID (declined/void excluded) and sum `approvedAmount`/`paidAmount`; because slices bill/approve *disjoint* entry subsets, Σ across a case's slices+final = case-recon `approvedToDate`/`paidToDate` — no double-count. ✅ **`A2-DEF-01` (Medium) FIXED:** the Admissions report counted *inpatient claims* as admissions → interim slices over-counted admissions N+1× + duplicate rows + skewed avg-LOS (and a fully-sliced case files no final, so it'd vanish). Rebuilt **episode-based** — sourced from `ClinicalCase` (billed = accrued) + direct inpatient claims (`caseId:null`), on-screen + export (`reports/[reportType]/page.tsx` `getAdmissionsData`, `export/route.ts` case "admissions"); tsc+lint clean, 789 green. **Open — `A2-OBS-02` (Low):** member-statements "Claims" *count* column still includes slices (money columns correct). **Gap — `A2-OBS-03`:** no user-facing GL trial-balance / claims-payable report (GL balanced at service level; `outstanding-bills` = premium-invoice AR, not claims-payable) → reports↔GL reconciliation needs DB/service access. **Residual:** live before/after on the VM (sliced fixtures) confirms the admissions count drop. | **P1→money-PASS, DEF-01 fixed** |
| A3 | **Maker/checker SoD via distinct personas (SET-09)** — ✅ **CODE-VERIFIED PASS 2026-07-21 + `A3-OBS-01` FIXED** | Adjudication + settlement were driven **service-level** with `matrixSatisfied`/service calls in the probes. The approval-matrix + maker≠checker segregation was not proven through the UI as **separate** finance users. | **Controls confirmed + UI-enforced:** `enforceSegregationOfDuties(maker,checker)` throws FORBIDDEN on equality — wired into the single-level claim path (`adjudicatorId` vs `reviewerId`), the multi-level approval-request (per level), and settlement (`batch.makerId===checkerId` throw + atomic SYS-1). The seeded matrix rule routes inpatient >200k → UNDERWRITER + `requiresDual`, so `decide()` opens an ApprovalRequest instead of approving (applied only after distinct checkers clear each level). The UI claim action passes `reviewerId: session.user.id` and NEVER sets `matrixSatisfied`/`systemDecision` — so it cannot bypass the matrix/SoD (the escape the probes used). Matches the comprehensive-UAT "dual+SoD enforced" result. ✅ **`A3-OBS-01` (Medium) FIXED:** `provisionTenant` seeded RBAC/COA/catalogs but NOT the approval matrix → a newly-*provisioned* tenant launched with an empty matrix, and `resolve()`→null makes `decide()` fail-open (single reviewer, any amount, no dual). Added idempotent `ApprovalMatrixService.seedForTenant` (mirrors the 3 seed rules incl. inpatient >200k dual) + wired into `provisionTenant`; the primary `medvex` tenant was already seeded via `seed.ts`. 2 unit tests + provisioning test updated; 791 green. **Residual:** live two-persona UI demo (distinct finance users) confirms the code-level control. | **P1→PASS, OBS-01 fixed** |
| A4 | **Concurrency (LIM-01 / LIM-03)** — ✅ **REAL-DB PROVEN 2026-07-21 (slice/PA race)** | Two racing approvals on one balance, and family-pool concurrency, were proven for the **outpatient** path (`benefit-race.integration.test.ts`) but not re-exercised on **inpatient slices** / the shared-family pool on the fixed build. | **PROVEN** — added `LIM-01 / A4` case to `tests/integration/interim-settlement.integration.test.ts`: two `Promise.allSettled` `decide()` calls on sibling slices of one case sharing one PA/hold, run against a **real Postgres under Serializable** (throwaway DB, 3× consecutive clean). Both commit; benefit `used` and PA `utilisedAmount` each advance by **exactly 2×ENTRY** — no double-spend, no lost update (the in-tx PA re-read stops the stale-snapshot rollback), no double-credit. `inSerializableTx` (Serializable + bounded P2034 retry) absorbs the conflict internally. **`A4-OBS-01` (Low, fail-safe):** the single shared `BenefitHold` can be reduced by only one slice's worth under *exact*-concurrent decides (up to one ENTRY stranded) even though `used`+PA advance correctly → UNDER-states availability, never over-spends, self-heals on hold expiry / case close; serial cuts release it fully. **Residual:** SHARED_FAMILY-pool race (LIM-03) uses the identical `computeAvailability`-under-Serializable mechanism (proven for a single shared balance by the outpatient race + this slice race); a dedicated family-pool case needs the `IP-UAT-FAM` fixtures. | **P2→PROVEN, OBS-01 logged** |
| A5 | **Full day-by-day scenario narratives (§12–17)** — ✅ **EXECUTED 2026-07-21 on the VM (latest main)** | Each of the six scenarios' **key binding test** was exercised, but not the full per-day detail across the admission with interim slices, recon, and close. | **RAN all six end-to-end on the VM (`aicare_uat`, latest main):** each scenario accrued day-by-day → interim slice(s) → **final close (all six `CLOSED_FILED`)** → **seven-ledger reconciliation CONSERVED 6/6** (`billed = onSlices + residual`, `approved = paid + outstanding`, `approved ≤ billed`, `paid ≤ approved`). Stroke = the complete exemplar (24 entries → 2 weekly slices 1.78M+1.335M → final 445k → settled → billed=approved=paid 3.56M). **Full billed→approved→PAID cycle on 4** (Stroke/Malaria/Burns/Foot) via a **real maker≠checker settlement run** (admin maker / finance checker, 102 claims/143M, SoD gate passed → voucher+GL posted → outstanding→0) — also confirms A3 settlement SoD live. Limit-binding partials proven (Malaria 9M/12M, Burns 100M/105.6M overall-cap, Foot 4M/9.6M). And the adjudication controls **fire live** in the narrative: Boda's slice blocked by the **PA-required gate** (CT-HEAD needs a PA — IPL-PA-01) and Maternity's by the **contract ceiling** (10M > payable) — correct enforcement, so those two close with un-approved slices (approved=0, still conserved). | **P2→EXECUTED** |
| A6 | **Provider-portal reconciliation parity (§11.9)** — ✅ **BUILT 2026-07-21** | The admin/ops reconciliation panel proves the money control; the **provider-portal** view of billed/approved/paid/outstanding/remaining-guarantee for an open sliced case is a documented fast-follow, not built. | **BUILT** `/provider/cases` (+ nav item): a provider-scoped list of admissions showing billed / approved / paid / outstanding / remaining-guarantee / slice-count per case, computed via the **same `CaseService.getCaseReconciliation`** the admin panel uses (parity to the shilling). Scoped by `providerId` from the session (A1-consistent — `where: { tenantId, providerId }`). **Data path real-DB verified** (throwaway-DB smoke: created a sliced case → provider fetch returned billed=approved=12k/outstanding=12k, and a different provider saw it `false`). Residual: live browser walkthrough as a provider persona (needs a provider login). | **P2→BUILT** |
