...
export const lifecycleService = {
  ...
  async initiateCoolingOffCancellation(memberId: string, tenantId: string, requestedById: string) {
    ...
    await auditChainService.append({
      ...
      tenantId,
      description: `Member ${member.memberNumber} cancelled within cooling-off window — full refund KES ${refundAmount.toLocaleString("en-KE")}`,
    });
  },
  ...
  async initiateStandardCancellation(
    ...
    await auditChainService.append({
      ...
      tenantId,
      description: `Member ${member.memberNumber} cancelled — pro-rata refund KES ${refundAmount.toLocaleString("en-KE")} (${remainingDays}d remaining)`,
    });
  },
  ...
  async recordPrincipalDeath(
    ...
    await auditChainService.append({
      ...
      tenantId,
      description: `Member ${member.memberNumber} deceased — ${member.dependents.length} dependant(s) suspended, pro-rata refund KES ${proRataRefund.toLocaleString("en-KE")}`,
    });
  },
