  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        ...
        <p className="text-brand-text-muted mt-1">Get help with your account, groups, or submissions.</p>
      </div>
        ...
        {[
          ...
          return (
            <a
              key={item.title}
              href={item.href}
              className="bg-white border border-[#EEEEEE] rounded-lg p-5 shadow-sm flex items-start gap-4 hover:border-brand-indigo/30 hover:shadow-md transition-all group"
            >
              <div className="w-10 h-10 bg-brand-indigo/10 rounded-lg flex items-center justify-center flex-shrink-0">
                <Icon size={20} className="text-brand-indigo" />
              </div>
              <div>
                <h2 className="font-bold text-brand-text-heading group-hover:text-brand-indigo transition-colors">{item.title}</h2>
                <p className="text-sm text-brand-text-body mt-0.5">{item.desc}</p>
                <p className="text-sm font-semibold text-brand-indigo mt-1">{item.action}</p>
              </div>
            </a>
          );
        })}
      ...
      <div className="bg-white border border-[#EEEEEE] rounded-lg p-6 shadow-sm space-y-4">
        <h2 className="font-bold text-brand-text-heading font-heading">Submit a Query</h2>
        ...
        {groups.length === 0 ? (
          ...
          <form action={submitBrokerSupportAction} className="space-y-4">
            <select
              name="groupId"
              required
              className="w-full border border-[#EEEEEE] rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-brand-indigo bg-white"
            >
            ...
            <input
              ...
              required
              placeholder="Subject"
              className="w-full border border-[#EEEEEE] rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-brand-indigo"
            />
            <textarea
              name="body"
              required
              ...
              className="w-full border border-[#EEEEEE] rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-brand-indigo"
            />
            <button type="submit" className="bg-brand-indigo hover:bg-brand-secondary text-white px-6 py-2 rounded-full font-semibold transition-colors">
              Send Query
            </button>
          </form>
        )}
      ...
      {recentRequests.length > 0 && (
        <div className="bg-white border border-[#EEEEEE] rounded-lg p-6 shadow-sm space-y-3">
          <h2 className="font-bold text-brand-text-heading font-heading">Recent Queries</h2>
          <div className="divide-y divide-[#EEEEEE]">
            {recentRequests.map((request) => (
              <div key={request.id} className="py-3 text-sm">
                <div className="flex items-center justify-between gap-3">
                  <p className="font-bold text-brand-text-heading">{request.subject}</p>
                  <span className="text-[10px] font-bold uppercase text-brand-text-muted">{request.status.replace(/_/g, " ")}</span>
                </div>
                <p className="text-xs text-brand-text-muted mt-1">{request.group.name} · {request.category.replace(/_/g, " ")}</p>
              </div>
            ))}
          </div>
        </div>
