  ...
  return (
    <div className="space-y-6 font-ui">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-bold uppercase text-avenue-text-muted">Member inbox</p>
          <h1 className="mt-1 text-2xl font-bold text-avenue-text-heading">Notifications</h1>
        ...
        <form action={markAllMemberNotificationsReadAction}>
          <button className="inline-flex items-center justify-center gap-2 rounded-[8px] border border-[#EEEEEE] bg-white px-4 py-2.5 text-sm font-semibold text-avenue-indigo shadow-sm hover:bg-[#F8F9FA]">
      ...
      <section className="rounded-[8px] border border-[#EEEEEE] bg-white p-5 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-bold uppercase text-avenue-text-muted">Unread</p>
            <p className="text-2xl font-bold tabular-nums text-avenue-text-heading">{inbox.unreadCount}</p>
          </div>
        </div>
      </section>
      ...
      <div className="space-y-3">
              ...
              <div className="grid gap-4 sm:grid-cols-[1fr_auto] sm:items-start">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="rounded-full bg-avenue-indigo/10 px-2.5 py-1 text-[10px] font-bold uppercase text-avenue-indigo">
                  ...
                  <h2 className="mt-2 text-base font-bold text-avenue-text-heading">{notification.title}</h2>
                  <p className="mt-1 text-sm text-avenue-text-muted">{notification.body}</p>
                  <p className="mt-2 text-xs text-avenue-text-muted">{formatDate(notification.createdAt)}</p>
                </div>
                <div className="flex items-center gap-2 sm:justify-end">
                  {unread && (
                    <form action={markMemberNotificationReadAction}>
                      <input type="hidden" name="notificationId" value={notification.id} />
                      <button className="rounded-[8px] border border-[#EEEEEE] bg-white px-3 py-2 text-xs font-semibold text-avenue-indigo hover:bg-[#F8F9FA]">
                        Mark read
                      </button>
                    </form>
                  )}
                  {notification.href && (
                    <Link href={notification.href} className="inline-flex items-center gap-1 rounded-[8px] bg-avenue-indigo px-3 py-2 text-xs font-semibold text-white">
        ...
        {inbox.notifications.length === 0 && (
          <div className="rounded-[8px] border border-dashed border-[#D6DCE5] bg-white p-8 text-center shadow-sm">
            <h2 className="text-base font-bold text-avenue-text-heading">No notifications yet</h2>
