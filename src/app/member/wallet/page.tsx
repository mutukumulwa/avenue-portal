  ...
  return (
    <div className="space-y-6 font-ui">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-bold uppercase text-avenue-text-muted">Member wallet</p>
          <h1 className="mt-1 text-2xl font-bold text-avenue-text-heading">M-Pesa Co-Contributions</h1>
        ...
        <div className="rounded-[8px] border border-[#EEEEEE] bg-white px-4 py-3 shadow-sm">
          ...
          <p className="mt-1 font-bold text-avenue-text-heading">{wallet.viewer.preferredPhone ?? "Not set"}</p>
        </div>
      </div>
      ...
      <section className="grid gap-3 md:grid-cols-3">
        <div className="rounded-[8px] border border-[#EEEEEE] bg-white p-4 shadow-sm">
          ...
          <p className="mt-2 text-2xl font-bold tabular-nums text-avenue-text-heading">
        ...
        <div className="rounded-[8px] border border-[#EEEEEE] bg-white p-4 shadow-sm">
          ...
          <p className="mt-2 text-2xl font-bold tabular-nums text-avenue-text-heading">{wallet.summary.openItemCount}</p>
        </div>
        <div className="rounded-[8px] border border-[#EEEEEE] bg-white p-4 shadow-sm">
          <div className="flex items-center gap-2 text-avenue-text-muted">
      ...
      <section className="rounded-[8px] border border-[#17A2B8]/25 bg-[#17A2B8]/5 p-4">
        <p className="text-sm font-bold text-avenue-text-heading">Facility authorization rule</p>
      ...
      <section className="space-y-3">
        <div>
          <h2 className="font-heading text-lg font-bold text-avenue-text-heading">Outstanding member share</h2>
          <p className="mt-1 text-sm text-avenue-text-muted">Balances are created from adjudicated care events and cleared only after callback confirmation.</p>
        </div>
              ...
              <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <p className="font-mono text-xs text-avenue-text-muted">{item.claimNumber}</p>
                    <span className="rounded-full border border-[#EEEEEE] px-2.5 py-1 text-[10px] font-bold uppercase text-avenue-text-muted">
                  ...
                  <h3 className="mt-2 text-base font-bold text-avenue-text-heading">{item.providerName}</h3>
                  <p className="mt-1 text-sm text-avenue-text-muted">
                    {item.memberName} · {item.serviceType.replace(/_/g, " ")} · {formatDate(item.dateOfService)}
                  </p>
                  <div className="mt-4 grid gap-3 sm:grid-cols-3">
                    <div>
                      <p className="text-xs font-bold uppercase text-avenue-text-muted">Due</p>
                      <p className="mt-1 font-bold tabular-nums text-avenue-text-heading">{currency.format(item.amountDue)}</p>
                    </div>
                    <div>
                      <p className="text-xs font-bold uppercase text-avenue-text-muted">Total share</p>
                      <p className="mt-1 font-bold tabular-nums text-avenue-text-heading">{currency.format(item.finalAmount)}</p>
                    </div>
                    <div>
                      <p className="text-xs font-bold uppercase text-avenue-text-muted">Collected</p>
                      <p className="mt-1 font-bold tabular-nums text-[#1F7A34]">{currency.format(item.amountCollected)}</p>
                    </div>
                  </div>
                  <Link href={`/member/utilization/${item.claimId}`} className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-avenue-indigo hover:underline">
                ...
                <div>
                  {pending && item.latestPayment ? (
                    <div className="rounded-[8px] border border-[#17A2B8]/25 bg-[#17A2B8]/5 p-4">
                      ...
                      <p className="mt-2 text-sm text-avenue-text-muted">
                    ...
                    <div className="rounded-[8px] border border-[#EEEEEE] bg-[#FAFBFD] p-4">
                      {latestStatus && !canRetry && (
                        <p className="mb-3 text-sm text-avenue-text-muted">This item is waiting for the next payment update.</p>
                      )}
                      {latestStatus && canRetry && (
        ...
        {wallet.outstanding.length === 0 && (
          <div className="rounded-[8px] border border-dashed border-[#D6DCE5] bg-white p-8 text-center shadow-sm">
            <h3 className="text-base font-bold text-avenue-text-heading">No outstanding member share</h3>
      ...
      <section className="space-y-3">
        <div>
          <h2 className="font-heading text-lg font-bold text-avenue-text-heading">Payment history</h2>
          <p className="mt-1 text-sm text-avenue-text-muted">Recent sandbox checkout attempts and callback outcomes.</p>
        </div>
        <div className="overflow-hidden rounded-[8px] border border-[#EEEEEE] bg-white shadow-sm">
          {wallet.payments.map((payment) => (
            <div key={payment.id} className="grid gap-3 border-b border-[#EEEEEE] p-4 last:border-b-0 md:grid-cols-[1.2fr_0.7fr_0.7fr]">
              <div>
                <p className="font-bold text-avenue-text-heading">{payment.providerName}</p>
                <p className="mt-1 text-sm text-avenue-text-muted">
                  {payment.memberName} · {payment.claimNumber} · {formatDate(payment.dateOfService)}
                </p>
                <p className="mt-1 font-mono text-xs text-avenue-text-muted">{payment.checkoutRequestId}</p>
              </div>
              <div>
                <p className="text-xs font-bold uppercase text-avenue-text-muted">Amount</p>
                <p className="mt-1 font-bold tabular-nums text-avenue-text-heading">{currency.format(payment.amount)}</p>
                <p className="mt-1 text-xs text-avenue-text-muted">{payment.phoneNumber}</p>
              </div>
              <div>
          ...
          {wallet.payments.length === 0 && (
            <p className="p-6 text-center text-sm text-avenue-text-muted">No wallet payment attempts yet.</p>
          )}
        </div>
      </section>
    </div>
