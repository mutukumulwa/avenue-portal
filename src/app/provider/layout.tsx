...
export default async function ProviderLayout({ children }: { children: React.ReactNode }) {
  ...
  // navigation from the caller's permissions. Only the safe, filtered groups
  // cross to the client — never the permissions/provider/branch context (§6.5).
  // P06: the groups are passed as groups — the bar renders direct links, menus
  // and the account menu from them — not flattened into one row.
  ...
  const navGroups = computeProviderNav(ctx.permissions, { flags: { contractView } });
