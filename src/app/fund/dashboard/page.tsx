...
export default async function FundDashboardPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-brand-text-heading font-heading">Fund Administrator Dashboard</h1>
          ...
          return (
            <div key={k.label} className="bg-white border border-[#EEEEEE] rounded-[8px] p-4 shadow-sm font-ui">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-xs font-bold uppercase tracking-wide text-brand-text-muted">{k.label}</p>
              </div>
      ...
      <div className="space-y-4">
            ...
            <div key={g.id} className={`bg-white border rounded-[8px] shadow-sm overflow-hidden ${isLow ? "border-[#DC3545]/30" : "border-[#EEEEEE]"}`}>
              ...
              <div className={`px-5 py-4 flex items-center justify-between ${isLow ? "bg-[#FFF5F5]" : "bg-white"} border-b border-[#EEEEEE]`}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="font-bold text-brand-text-heading font-heading">{g.name}</h2>
                    <p className="text-xs text-brand-text-muted mt-0.5">
                      {g.members.length} active members · Period: {new Date(acc.periodStartDate).toLocaleDateString("en-UG", { month: "short", year: "numeric" })} – {new Date(acc.periodEndDate).toLocaleDateString("en-UG", { month: "short", year: "numeric" })}
                    </p>
                  </div>
                </div>
                <Link href={`/fund/${g.id}`}
              ...
              <div className="px-5 py-4 grid md:grid-cols-4 gap-6 font-ui">
                ...
                <div className="md:col-span-2">
                  <div className="flex justify-between text-xs font-semibold text-brand-text-muted mb-1">
              ...
              {txns.length > 0 && (
                <div className="border-t border-[#EEEEEE] px-5 py-3">
                  <p className="text-[10px] font-bold uppercase text-brand-text-muted mb-2">Recent Activity</p>
        ...
        {groups.length === 0 && (
          <div className="bg-white border border-[#EEEEEE] rounded-[8px] p-12 text-center text-brand-text-muted">
      ...
      {recentClaims.length > 0 && (
        <div className="bg-white border border-[#EEEEEE] rounded-[8px] shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-[#EEEEEE] flex items-center gap-2">
            ...
            <h2 className="font-bold text-brand-text-heading font-heading">Large Claims (≥ KES 50,000)</h2>
          </div>
            ...
            <tbody className="divide-y divide-[#EEEEEE]">
              {recentClaims.map(c => (
                <tr key={c.id} className="hover:bg-[#F8F9FA]">
                  <td className="px-4 py-2.5 font-mono text-xs text-brand-indigo">{c.claimNumber}</td>
                  <td className="px-4 py-2.5">{c.member.firstName} {c.member.lastName}</td>
                  <td className="px-4 py-2.5 text-brand-text-muted">{c.member.group.name}</td>
                  <td className="px-4 py-2.5 text-brand-text-muted">{c.benefitCategory.replace(/_/g, " ")}</td>
                  <td className="px-4 py-2.5 text-right font-mono font-bold text-[#DC3545]">
                    {Number(c.approvedAmount).toLocaleString("en-UG")}
                  </td>
                  <td className="px-4 py-2.5 text-brand-text-muted text-xs">
