...
function statusTone(status: AnalyticsAlertStatus) {
  ...
  if (status === "ACKNOWLEDGED") return "bg-avenue-indigo/10 text-avenue-indigo";
...
function WorkloadCards({ data }: { data: AlertsData }) {
  const cards = [
    ...
    { label: "Acknowledged", value: data.statusCounts.ACKNOWLEDGED ?? 0, tone: "text-avenue-indigo bg-avenue-indigo/10" },
        ...
        <div key={card.label} className="rounded-[8px] border border-[#EEEEEE] bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-2 text-2xl font-bold tabular-nums text-avenue-text-heading">{card.value.toLocaleString()}</p>
            </div>
      ...
      <div className="flex flex-wrap gap-2">
        {statusTabs.map((tab) => {
          ...
          return (
            <Link
              ...
              className={`rounded-full px-3 py-1 text-[13px] font-bold ${active ? "bg-avenue-indigo text-white" : "bg-avenue-bg-alt text-avenue-text-heading hover:text-avenue-indigo"}`}
            >
              {tab.label}
            </Link>
          );
        ...
        {Object.values(AnalyticsAlertSeverity).map((severity) => (
          <Link
            ...
            className={`rounded-full px-3 py-1 text-[13px] font-bold ${params.severity === severity ? severityTone(severity) : "bg-avenue-bg-alt text-avenue-text-heading hover:text-avenue-indigo"}`}
          >
            {severity}
          </Link>
        ))}
    ...
    <div className="grid gap-2">
      {alert.status === "OPEN" && (
        <form action={acknowledgeAnalyticsAlertAction}>
          ...
          <button className="inline-flex w-full items-center justify-center gap-2 rounded-[8px] border border-[#EEEEEE] px-3 py-2 text-sm font-semibold text-avenue-text-heading hover:border-avenue-indigo hover:text-avenue-indigo">
      ...
      <form action={resolveAnalyticsAlertAction} className="grid gap-2">
        ...
        <input
          name="resolutionNote"
          className="h-9 rounded-[8px] border border-[#EEEEEE] px-3 text-sm outline-none focus:border-avenue-indigo"
          placeholder="Resolution note"
        />
        <button className="inline-flex w-full items-center justify-center gap-2 rounded-[8px] bg-avenue-indigo px-3 py-2 text-sm font-semibold text-white hover:bg-avenue-indigo/90">
    ...
    <div className="grid gap-4 px-5 py-4 lg:grid-cols-[1fr_260px]">
      <div className="min-w-0">
        <div className="mb-2 flex flex-wrap items-center gap-2">
          ...
          <span className="rounded-full bg-avenue-bg-alt px-2 py-1 text-[13px] font-bold text-avenue-text-heading">{alert.type.replace(/_/g, " ")}</span>
        </div>
        <h2 className="font-heading text-lg font-bold text-avenue-text-heading">{alert.title}</h2>
        <p className="mt-1 text-sm leading-relaxed text-avenue-text-muted">{alert.message}</p>

        <div className="mt-4 grid gap-3 text-[13px] md:grid-cols-3">
          <div>
            <p className="font-bold uppercase tracking-normal text-avenue-text-muted">Metric</p>
            <p className="mt-1 font-semibold tabular-nums text-avenue-text-heading">
              {alert.metricKey ?? "Signal"}: {formatMetric(alert.metricValue, alert.metricKey)}
            </p>
          </div>
          <div>
            <p className="font-bold uppercase tracking-normal text-avenue-text-muted">Threshold</p>
            <p className="mt-1 font-semibold tabular-nums text-avenue-text-heading">
              {alert.thresholdValue ? formatMetric(alert.thresholdValue, alert.metricKey) : "Not set"}
            </p>
          </div>
          <div>
            <p className="font-bold uppercase tracking-normal text-avenue-text-muted">Raised</p>
            <p className="mt-1 font-semibold text-avenue-text-heading">{formatDate(alert.createdAt)}</p>
          </div>
        </div>

        <div className="mt-4 flex flex-wrap gap-2 text-[13px]">
          {alert.groupId && (
            <Link href={`/analytics/schemes/${alert.groupId}`} className="inline-flex items-center gap-1 rounded-full bg-avenue-indigo/10 px-2 py-1 font-semibold text-avenue-indigo hover:underline">
          ...
          {alert.memberId && (
            <Link href={`/members/${alert.memberId}`} className="rounded-full bg-avenue-bg-alt px-2 py-1 font-semibold text-avenue-text-heading hover:text-avenue-indigo">
              {alert.memberNumber ?? alert.memberName ?? "Member"}
            </Link>
          )}
          {alert.intermediaryName && (
            <span className="rounded-full bg-avenue-bg-alt px-2 py-1 font-semibold text-avenue-text-heading">
              {alert.intermediaryName}
            </span>
          )}
        </div>
        ...
        {alert.resolutionNote && (
          <p className="mt-4 rounded-[8px] bg-[#F8F9FA] px-3 py-2 text-sm text-avenue-text-muted">{alert.resolutionNote}</p>
        )}
      </div>
    ...
    <div className="space-y-6 font-ui">
      <div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <h1 className="font-heading text-3xl font-bold text-avenue-text-heading">Analytics Alert Inbox</h1>
          <p className="text-avenue-text-muted">Operational queue for MLR drift, provider anomalies, renewal risk, and contribution signals.</p>
        </div>
      </div>
      ...
      <div className="rounded-[8px] border border-[#EEEEEE] bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-[#EEEEEE] px-5 py-4">
          <div>
            <h2 className="font-heading text-lg font-bold text-avenue-text-heading">Alert Stream</h2>
            <p className="text-sm text-avenue-text-muted">{data.alerts.length.toLocaleString()} alerts match the current filter.</p>
          </div>
          {params.groupId && (
            <Link href="/analytics/alerts" className="text-sm font-semibold text-avenue-indigo hover:underline">
              Clear scheme filter
            </Link>
          )}
        </div>
        <div className="divide-y divide-[#EEEEEE]">
          {data.alerts.length === 0 && (
            <p className="px-5 py-10 text-center text-sm text-avenue-text-muted">No analytics alerts match this queue filter.</p>
          )}
