...
function MetricStrip({
  ...
  const metrics = [
    ...
    },
    {
      ...
      detail: "Active lives",
      icon: Users,
      tone: "text-avenue-indigo bg-avenue-indigo/10",
    },
    {
  ...
  return (
    <div className="flex h-9 items-end gap-1">
      {points.map((point, index) => (
        <span
          key={index}
          className="block w-2 rounded-t bg-avenue-indigo/60"
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[860px] text-left text-sm">
          ...
          <tbody className="divide-y divide-[#EEEEEE]">
            {schemes.length === 0 && (
              <tr>
                <td colSpan={7} className="px-5 py-8 text-center text-sm text-avenue-text-muted">
    ...
    <div className="rounded-[8px] border border-[#EEEEEE] bg-white shadow-sm">
      <div className="flex items-center justify-between border-b border-[#EEEEEE] px-5 py-4">
        <div>
          ...
          <p className="text-sm text-avenue-text-muted">Ranked by case-mix-adjusted cost in the latest refreshed period.</p>
        </div>
    ...
    <div className="rounded-[8px] border border-[#EEEEEE] bg-white shadow-sm">
      <div className="flex items-center justify-between border-b border-[#EEEEEE] px-5 py-4">
        <div>
          ...
          <p className="text-sm text-avenue-text-muted">Schemes due in the next 90 days with pricing pressure signals.</p>
        </div>
        ...
        {renewals.slice(0, 5).map((renewal) => (
          <div key={renewal.analysisId} className="px-5 py-4">
            ...
            <div className="grid grid-cols-3 gap-2 text-xs">
              ...
              <div>
                <p className="text-avenue-text-muted">Target</p>
                <p className="font-bold tabular-nums text-avenue-text-heading">{formatPercent(renewal.targetMlr)}</p>
              </div>
              <div>
                <p className="text-avenue-text-muted">Adjustment</p>
