  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        <Link href="/overrides" className="text-brand-text-muted hover:text-brand-indigo transition-colors" aria-label="Back to overrides">
          <ArrowLeft size={20} />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-brand-text-heading font-heading">Override Patterns</h1>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          { label: "SLA Breached",    value: summary.slaBreached,    color: summary.slaBreached > 0 ? "text-[#DC3545] font-bold" : "text-brand-text-muted" },
      ...
      <div className="bg-white border border-[#EEEEEE] rounded-[8px] shadow-sm p-5 space-y-4">
        ...
        {patterns.length === 0 ? (
          <p className="text-sm text-brand-text-muted text-center py-6">No override requests in this period.</p>
        ) : (
              ...
              return (
                <div key={p.makerId} className="space-y-1">
                  <div className="flex items-center justify-between text-sm">
                    <span className="font-semibold text-brand-text-heading">{p.makerName}</span>
                    <div className="flex items-center gap-3 text-xs text-brand-text-muted">
                      ...
                      <span className="font-bold text-brand-text-heading">{p.total} total</span>
                    </div>
                  </div>
                  {/* Bar */}
                  <div className="flex items-center gap-2">
                    <div className="flex-1 bg-[#EEEEEE] rounded-full h-3 overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all ${p.total > 10 ? "bg-[#DC3545]" : "bg-brand-indigo"}`}
                        style={{ width: `${barWidth}%` }}
                      />
                    </div>
                    <span className="text-[11px] text-brand-text-muted w-16 text-right">{approvalRate}% approved</span>
                  </div>
                  {/* Type breakdown */}
                  <div className="flex flex-wrap gap-1 pl-0">
                    {Object.entries(p.byType).sort(([, a], [, b]) => b - a).map(([type, count]) => (
                      <span key={type} className="text-[10px] bg-[#F8F9FA] border border-[#EEEEEE] px-2 py-0.5 rounded-full text-brand-text-muted">
          ...
          <div className="grid grid-cols-3 gap-2">
            {summary.byType.map(({ type, count }) => (
              <div key={type} className="border border-[#EEEEEE] rounded-[6px] px-3 py-2">
                <p className="text-[11px] text-brand-text-muted">{type.replace(/_/g, " ")}</p>
                <p className="font-bold text-brand-text-heading mt-0.5">{count}</p>
              </div>
            ))}
          </div>
        </div>
