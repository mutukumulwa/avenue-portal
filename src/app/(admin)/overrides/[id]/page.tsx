    ...
    <div className="p-6 max-w-3xl mx-auto space-y-5">
      ...
      {isPending && record.slaDeadlineAt && (
        <div className={`rounded-[8px] p-3 flex items-center gap-2 border ${slaBreached ? "bg-[#DC3545]/10 border-[#DC3545]/30" : "bg-[#28A745]/10 border-[#28A745]/30"}`}>
          ...
          <span className="ml-auto text-xs text-avenue-text-muted">
      ...
      <div className="bg-white border border-[#EEEEEE] rounded-[8px] shadow-sm p-5 space-y-4">
        <h2 className="font-semibold text-avenue-text-heading text-sm border-b border-[#EEEEEE] pb-2">Override Details</h2>
        ...
        <div>
          <p className="text-xs text-avenue-text-muted">Justification</p>
          <p className="text-sm text-avenue-text-body mt-1 whitespace-pre-wrap">{record.justification}</p>
        </div>
      </div>
      ...
      {(record.preState || record.postState) && (
        <div className="bg-white border border-[#EEEEEE] rounded-[8px] shadow-sm p-5 space-y-3">
          <h2 className="font-semibold text-avenue-text-heading text-sm border-b border-[#EEEEEE] pb-2">State Snapshot</h2>
          <div className="grid grid-cols-2 gap-4 text-xs font-mono">
            <div>
              <p className="font-bold text-avenue-text-muted mb-1 uppercase text-[10px]">Before</p>
              <pre className="bg-[#F8F9FA] rounded p-2 overflow-x-auto text-[10px]">
                {record.preState ? JSON.stringify(record.preState, null, 2) : "—"}
              </pre>
            </div>
            <div>
              <p className="font-bold text-avenue-text-muted mb-1 uppercase text-[10px]">After</p>
              <pre className="bg-[#F8F9FA] rounded p-2 overflow-x-auto text-[10px]">
      ...
      {makerHistory.length > 0 && (
        <div className="bg-white border border-[#EEEEEE] rounded-[8px] shadow-sm p-5 space-y-3">
          <div className="flex items-center gap-2">
            <Shield size={14} className="text-avenue-indigo" />
            <h2 className="font-semibold text-avenue-text-heading text-sm">
              {record.maker.firstName}&apos;s Recent Override History
            </h2>
            <span className="text-[10px] text-avenue-text-muted ml-auto">Shown for pattern detection</span>
          </div>
          <div className="space-y-1.5">
            {makerHistory.map((h) => (
              <div key={h.id} className="flex items-center gap-3 text-xs">
                ...
                <span className="text-avenue-text-body">{TYPE_LABEL[h.overrideType] ?? h.overrideType}</span>
                <span className="text-avenue-text-muted ml-auto">{new Date(h.createdAt).toLocaleDateString("en-KE")}</span>
                <Link href={`/overrides/${h.id}`} className="text-avenue-indigo hover:underline">→</Link>
              </div>
            ))}
          </div>
        </div>
