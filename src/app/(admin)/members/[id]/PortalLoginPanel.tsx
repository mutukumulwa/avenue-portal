...
export function PortalLoginPanel({ memberId, defaultEmail, portalUser }: Props) {
  ...
  const input = "border border-[#EEEEEE] rounded-md px-3 py-2 text-sm outline-none focus:border-brand-indigo bg-white";

  return (
    <div className="bg-white border border-[#EEEEEE] rounded-[8px] p-5 shadow-sm space-y-3">
      <div>
        <h3 className="text-sm font-bold text-brand-text-heading">Member Portal Login</h3>
        <p className="text-xs text-brand-text-muted mt-0.5">Create or reset the member self-service account from inside the app.</p>
      </div>
      {portalUser ? (
        <div className="space-y-3">
          <div className="flex flex-wrap items-center gap-2 text-sm">
            <span className="font-semibold text-brand-text-heading">{portalUser.email}</span>
          ...
          <form action={resetAction} className="flex flex-wrap items-end gap-2">
            <label className="space-y-1">
              <span className="block text-xs font-bold uppercase text-brand-text-muted">New Temporary Password</span>
              <input name="password" type="password" minLength={8} required className={input} />
            </label>
            <button disabled={resetPending} className="px-4 py-2 rounded-full bg-brand-indigo text-white text-xs font-bold hover:bg-brand-secondary disabled:opacity-50">
        ...
        <form action={createAction} className="grid md:grid-cols-[1fr_1fr_auto] gap-2 items-end">
          ...
          <label className="space-y-1">
            <span className="block text-xs font-bold uppercase text-brand-text-muted">Login Email</span>
            <input name="email" type="email" required defaultValue={defaultEmail ?? ""} className={input} />
          </label>
          <label className="space-y-1">
            <span className="block text-xs font-bold uppercase text-brand-text-muted">Temporary Password</span>
            <input name="password" type="password" minLength={8} required className={input} />
          </label>
          <button disabled={createPending} className="px-4 py-2 rounded-full bg-brand-indigo text-white text-xs font-bold hover:bg-brand-secondary disabled:opacity-50">
