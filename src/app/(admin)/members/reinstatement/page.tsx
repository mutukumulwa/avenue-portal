  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-avenue-text-heading font-heading">Reinstatement Requests</h1>
          <p className="text-avenue-text-muted text-sm mt-0.5">{requests.length} pending</p>
        </div>
      </div>
      ...
      <div className="bg-white border border-[#EEEEEE] rounded-lg shadow-sm overflow-hidden">
        {requests.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <p className="text-avenue-text-muted">No pending reinstatement requests.</p>
          </div>
        ) : (
              ...
              {requests.map(r => (
                <tr key={r.id} className="hover:bg-[#F8F9FA] align-top">
                  <td className="px-5 py-4">
                    <Link href={`/members/${r.member.id}`} className="font-semibold text-avenue-text-heading hover:text-avenue-indigo transition-colors">
                      {r.member.firstName} {r.member.lastName}
                    </Link>
                    <p className="text-avenue-text-muted font-mono text-xs mt-0.5">{r.member.memberNumber}</p>
                  </td>
                  ...
                  <td className="px-5 py-4 font-semibold text-avenue-indigo">{Number(r.catchUpAmount).toLocaleString()}</td>
                  <td className="px-5 py-4 text-avenue-text-muted">{new Date(r.requestDate).toLocaleDateString("en-KE")}</td>
                  <td className="px-5 py-4">
