      ...
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-[#EEEEEE]">
          {exclusions.map(e =>
            editingId === e.id ? (
              ...
              <tr key={e.id} className="hover:bg-[#F8F9FA]">
                <td className="px-5 py-2.5 font-mono text-xs text-brand-text-muted">{e.cptCode ?? "—"}</td>
                <td className="px-5 py-2.5 font-medium text-brand-text-heading">{e.serviceName}</td>
                <td className="px-5 py-2.5 text-brand-text-muted">{e.reason ?? "—"}</td>
                {editable && (
                  <td className="px-5 py-2.5">
                    <div className="flex gap-2.5 justify-end">
                      <button onClick={() => { setEditingId(e.id); setAdding(false); }} className="text-brand-text-muted hover:text-brand-indigo transition-colors" title="Edit">
                        <Pencil size={13} />
                      </button>
          ...
          {exclusions.length === 0 && !adding && (
            <tr>
              <td colSpan={editable ? 4 : 3} className="px-5 py-8 text-center text-sm text-brand-text-muted">
