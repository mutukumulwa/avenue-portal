        ...
        <div className="flex items-center gap-3">
          ...
          {canIssue && (
            <form action={issueQuoteAction}>
              ...
              <button type="submit"
                className="bg-avenue-indigo hover:bg-avenue-secondary text-white px-5 py-2 rounded-full text-sm font-semibold transition-colors flex items-center gap-2 shadow-sm">
                <Send size={15} />
                Issue Quotation
              </button>
            </form>
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          <form action={buildQuoteAction} className="bg-white border border-[#EEEEEE] rounded-[8px] shadow-sm p-5 space-y-4">
            <input type="hidden" name="quotationId" value={id} />
            <div className="flex items-center justify-between">
              <h2 className="font-semibold text-avenue-text-heading text-sm">Pricing Parameters</h2>
              {hasLineItems && (
            ...
            <div>
              <label className="block text-xs font-semibold text-avenue-text-muted mb-1">Group size discount (%)</label>
              ...
              <p className="text-[11px] text-avenue-text-muted mt-0.5">&gt;100 lives = 5%, &gt;200 = 10%</p>
            </div>

            <div>
              <label className="block text-xs font-semibold text-avenue-text-muted mb-1">Loyalty discount (%)</label>
            ...
            <div>
              <label className="block text-xs font-semibold text-avenue-text-muted mb-1">Custom discount (%)</label>
              ...
              <input name="customDiscountDescription" type="text"
                ...
                className="w-full border border-[#EEEEEE] rounded-[6px] px-3 py-2 text-sm mt-1 focus:ring-1 focus:ring-avenue-indigo focus:outline-none" />
            </div>

            <div className="border-t border-[#EEEEEE] pt-3 space-y-2">
              <p className="text-xs font-semibold text-avenue-text-muted uppercase tracking-wide">Ancillary Fees</p>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-[11px] text-avenue-text-muted mb-1">Card fee / life (KES)</label>
                ...
                <div>
                  <label className="block text-[11px] text-avenue-text-muted mb-1">Welcome pack / life (KES)</label>
            ...
            <div>
              <label className="block text-xs font-semibold text-avenue-text-muted mb-1">Quote validity (days)</label>
            ...
            <button type="submit"
              className="w-full flex items-center justify-center gap-2 bg-avenue-indigo hover:bg-avenue-secondary text-white py-2.5 rounded-[8px] text-sm font-semibold transition-colors">
              <RefreshCw size={15} />
          ...
          {quotation.versions.length > 0 && (
            <div className="bg-white border border-[#EEEEEE] rounded-[8px] shadow-sm p-5">
              <h2 className="font-semibold text-avenue-text-heading text-sm mb-3">Version History</h2>
              <div className="space-y-2">
                {quotation.versions.map((v) => (
                  <div key={v.id} className="flex items-center justify-between text-xs">
                    <span className="text-avenue-text-muted">v{v.versionNumber} — {v.status}</span>
        ...
        <div className="col-span-2 space-y-4">
          {!hasLineItems ? (
            <div className="bg-white border border-[#EEEEEE] rounded-[8px] shadow-sm p-12 text-center">
              <Calculator size={36} className="mx-auto mb-3 text-avenue-text-muted opacity-30" />
              <p className="text-avenue-text-muted text-sm">Fill in the pricing parameters on the left and click <strong>Build Pricing</strong> to generate the contribution schedule.</p>
            </div>
          ) : (
            <>
              {/* KPI cards */}
              <div className="grid grid-cols-3 gap-3">
                {[
                  { label: "Total Contribution",   value: fmt(totalContribution),                               color: "text-avenue-indigo" },
                  ...
                  <div key={k.label} className="bg-white border border-[#EEEEEE] rounded-[8px] p-4 shadow-sm">
                    <p className="text-[11px] text-avenue-text-muted font-bold uppercase">{k.label}</p>
                ...
                <table className="w-full text-sm text-left">
                  ...
                  <tbody className="divide-y divide-[#EEEEEE]">
                    ...
                    {baseLines.map((l) => (
                      <tr key={l.id} className="hover:bg-[#F8F9FA]">
                        <td className="px-5 py-3 text-avenue-text-body">
                          {l.lifeName && <span className="text-[10px] font-mono text-avenue-text-muted mr-2">{l.lifeName}</span>}
                    ...
                    {loadingLines.length > 0 && (
                      <tr className="bg-[#f8f9ff] border-y border-avenue-indigo/20">
                        <td className="px-5 py-2.5 font-semibold text-avenue-indigo text-xs uppercase tracking-wide">Sub-total (post-loadings)</td>
                        <td className="px-5 py-2.5 text-right font-bold text-avenue-indigo">
                    ...
                    <tr className="bg-[#F8F9FA]">
                      <td colSpan={2} className="px-5 py-1.5 text-[11px] font-bold uppercase text-avenue-text-muted tracking-wide">
                    ...
                    {feeLines.map((l) => (
                      <tr key={l.id} className="hover:bg-[#F8F9FA]">
                        <td className="px-5 py-3 text-avenue-text-body">{l.description}</td>
                        <td className="px-5 py-3 text-right font-semibold text-avenue-text-heading">
                          {fmt(Number(l.netAmount))}
                        </td>
                      </tr>
