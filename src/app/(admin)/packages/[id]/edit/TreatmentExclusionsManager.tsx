...
import { createTreatmentExclusionAction, deleteTreatmentExclusionAction } from "./actions";
...
export function TreatmentExclusionsManager({
  packageVersionId,
  initialRules,
}: {
  packageVersionId: string;
  initialRules: ExclusionListItem[];
}) {
  ...
  const handleDelete = (id: string) => {
    if (!confirm("Remove this treatment exclusion?")) return;
  ...
  return (
    <div className="bg-white border border-[#EEEEEE] rounded-[8px] p-5 shadow-sm space-y-4 mt-6">
      <div className="flex items-center justify-between border-b border-[#EEEEEE] pb-2">
        <h2 className="font-bold text-brand-text-heading font-heading">Treatment Exclusions</h2>
        <button
          type="button"
      ...
      <p className="text-xs text-brand-text-muted">
        Structured, version-immutable exclusions. Absolute rules always exclude; conditional rules exclude unless their
        exception applies (e.g. cosmetic surgery excluded unless reconstructive after a covered trauma). Members and
        providers see only the member-safe explanation.
      </p>
        ...
        <form action={formAction} className="bg-gray-50 border border-gray-200 rounded p-4 space-y-4">
          ...
          <div>
            ...
            <textarea name="memberSafeExplanation" rows={2} className="w-full border p-2 rounded text-sm"
              placeholder="Cosmetic procedures are not covered unless reconstructive following a covered injury."
          ...
          <details className="text-xs">
            ...
            <div className="grid grid-cols-2 gap-4 mt-2">
              <div>
                ...
                <input type="text" name="sourceClause" className="w-full border p-2 rounded text-sm" placeholder="Policy §7.3(b)" />
              </div>
              <div>
          ...
          {initialRules.map((r) => (
            <div key={r.id} className="border border-gray-200 rounded p-4 flex justify-between items-start">
              ...
              <button type="button" onClick={() => handleDelete(r.id)} disabled={isPending}
                aria-label={`Remove ${r.ruleCategory.replace(/_/g, " ").toLowerCase()} treatment exclusion effective ${fmt(r.effectiveFrom)}`}
      ...
      {initialRules.length === 0 && !adding && (
        <div className="text-center p-6 text-gray-500 border-2 border-dashed border-gray-200 rounded">
          No treatment exclusions configured for this package version.
        </div>
      )}
    </div>
  );
