  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-brand-text-heading font-heading">Resolve Query</h1>
          <p className="text-brand-text-body font-mono text-sm mt-0.5">{req.id.toUpperCase().slice(-8)}</p>
        </div>
      </div>
      ...
      <div className="bg-white border border-[#EEEEEE] rounded-[8px] shadow-sm p-6 space-y-6">
        <div className="flex justify-between items-start border-b border-[#EEEEEE] pb-6">
          <div>
            <h2 className="text-xl font-bold text-brand-text-heading">{req.subject}</h2>
            <div className="flex items-center gap-4 mt-2 text-sm">
              ...
              <span className="text-brand-text-muted flex items-center gap-1"><Clock size={14} /> {new Date(req.createdAt).toLocaleString()}</span>
            </div>
          </div>
          <div className="text-right">
            <p className="font-bold text-brand-indigo">{req.group.name}</p>
            <p className="text-sm text-brand-text-muted">{req.submittedBy.firstName} {req.submittedBy.lastName}</p>
            <p className="text-xs text-brand-text-muted">{req.submittedBy.email}</p>
          </div>
        </div>

        <div className="bg-[#F8F9FA] p-5 rounded-lg border border-[#EEEEEE] text-sm text-brand-text-body whitespace-pre-wrap">
          {req.body}
        </div>
        ...
        {req.status === "RESOLVED" && req.response ? (
          ...
          <div className="border-t border-[#EEEEEE] pt-6">
            <h3 className="font-bold text-brand-text-heading mb-3">Provide Resolution</h3>
            <form action={resolveServiceRequestAction} className="space-y-4">
              ...
              <textarea 
                ...
                name="response" 
                rows={6} 
                className="w-full border border-[#EEEEEE] rounded-lg p-4 text-sm focus:outline-none focus:border-brand-indigo transition-colors"
