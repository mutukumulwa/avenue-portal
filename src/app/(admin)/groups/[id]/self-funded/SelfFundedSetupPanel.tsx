...
export function SelfFundedSetupPanel({
  ...
  return (
    <form action={handleSubmit} className="bg-white border border-[#EEEEEE] rounded-[8px] p-5 shadow-sm space-y-3">
      <div>
        <h3 className="text-sm font-bold text-brand-text-heading">Self-Funded Administration</h3>
        <p className="text-xs text-brand-text-muted mt-0.5">Convert this group and assign fund administrators from inside the app.</p>
      </div>
      {error && <p className="text-xs text-[#DC3545]">{error}</p>}
      <div className="grid md:grid-cols-4 gap-3">
        <label className="space-y-1">
          <span className="block text-xs font-bold uppercase text-brand-text-muted">Funding Mode</span>
          <input value={fundingMode} readOnly className="w-full border border-[#EEEEEE] rounded-md px-3 py-2 text-sm bg-[#F8F9FA]" />
        </label>
        <label className="space-y-1">
          <span className="block text-xs font-bold uppercase text-brand-text-muted">Admin Fee Method</span>
        ...
        <label className="space-y-1">
          <span className="block text-xs font-bold uppercase text-brand-text-muted">Admin Fee Rate</span>
        ...
        <label className="space-y-1">
          <span className="block text-xs font-bold uppercase text-brand-text-muted">Minimum Balance</span>
      ...
      <label className="block space-y-1">
        <span className="block text-xs font-bold uppercase text-brand-text-muted">Fund Administrators</span>
      ...
      <div className="flex justify-end">
        <button disabled={pending} className="px-4 py-2 rounded-full bg-brand-indigo text-white text-xs font-bold hover:bg-brand-secondary disabled:opacity-50">
