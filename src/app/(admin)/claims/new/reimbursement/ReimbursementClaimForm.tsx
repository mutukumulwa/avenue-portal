  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-white border border-[#EEEEEE] rounded-[8px] p-5 shadow-sm space-y-4">
        <h3 className="font-bold text-brand-text-heading text-sm">Encounter Details</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-1">
            <label className="text-xs font-semibold text-brand-text-muted uppercase">Member</label>
            <select name="memberId" required className="w-full border border-[#EEEEEE] rounded-md px-3 py-2 text-sm outline-none focus:border-brand-indigo">
          ...
          <div className="space-y-1">
            <label className="text-xs font-semibold text-brand-text-muted uppercase">Provider (where member was treated)</label>
            <select name="providerId" required className="w-full border border-[#EEEEEE] rounded-md px-3 py-2 text-sm outline-none focus:border-brand-indigo">
          ...
          <div className="space-y-1">
            <label className="text-xs font-semibold text-brand-text-muted uppercase">Date of Service</label>
            <input name="dateOfService" type="date" required className="w-full border border-[#EEEEEE] rounded-md px-3 py-2 text-sm outline-none focus:border-brand-indigo" />
          </div>
          <div className="space-y-1">
            <label className="text-xs font-semibold text-brand-text-muted uppercase">Benefit Category</label>
            <select name="benefitCategory" required className="w-full border border-[#EEEEEE] rounded-md px-3 py-2 text-sm outline-none focus:border-brand-indigo">
              <option value="">Select…</option>
              {BENEFIT_CATEGORIES.map(c => <option key={c} value={c}>{c.replace(/_/g, " ")}</option>)}
            </select>
          </div>
          <div className="space-y-1">
            <label className="text-xs font-semibold text-brand-text-muted uppercase">Attending Doctor</label>
            <input name="attendingDoctor" type="text" className="w-full border border-[#EEEEEE] rounded-md px-3 py-2 text-sm outline-none focus:border-brand-indigo" />
          </div>
          <div className="space-y-1">
            <label className="text-xs font-semibold text-brand-text-muted uppercase">Provider Invoice No.</label>
            <input name="invoiceNumber" type="text" placeholder="e.g. INV-2025-00123" className="w-full border border-[#EEEEEE] rounded-md px-3 py-2 text-sm outline-none focus:border-brand-indigo" />
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-1">
            <label className="text-xs font-semibold text-brand-text-muted uppercase">Primary Diagnosis Code</label>
            <input name="diagnosisCode" type="text" placeholder="e.g. J18.9" className="w-full border border-[#EEEEEE] rounded-md px-3 py-2 text-sm outline-none focus:border-brand-indigo" />
          </div>
          <div className="space-y-1">
            <label className="text-xs font-semibold text-brand-text-muted uppercase">Diagnosis Description</label>
            <input name="diagnosisDesc" type="text" required placeholder="e.g. Pneumonia" className="w-full border border-[#EEEEEE] rounded-md px-3 py-2 text-sm outline-none focus:border-brand-indigo" />
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-[#EEEEEE] rounded-[8px] p-5 shadow-sm space-y-3">
                ...
                <tr key={i}>
                  <td className="py-2 pr-2">
                    <select value={line.serviceCategory} onChange={e => updateLine(i, "serviceCategory", e.target.value)}
                      className="w-full border border-[#EEEEEE] rounded px-2 py-1.5 text-xs outline-none focus:border-brand-indigo">
                  ...
                  <td className="py-2 text-right font-mono text-xs font-semibold text-brand-text-heading pr-2">
        ...
        <div className="flex items-center justify-between pt-1">
          <button type="button" onClick={() => setLines(prev => [...prev, emptyLine()])}
            className="flex items-center gap-1 text-xs text-brand-indigo hover:text-brand-secondary transition-colors">
      ...
      <div className="bg-white border border-[#EEEEEE] rounded-[8px] p-5 shadow-sm space-y-4">
        <h3 className="font-bold text-brand-text-heading text-sm">Reimbursement Payment Details</h3>
        <p className="text-xs text-brand-text-muted">Where should the insurer send the reimbursement?</p>
        ...
        {payMethod === "mpesa" ? (
          <div className="space-y-1 max-w-xs">
            <label className="text-xs font-semibold text-brand-text-muted uppercase">M-Pesa Phone Number</label>
            <input name="mpesaPhone" type="tel" required placeholder="e.g. 0712345678" className="w-full border border-[#EEEEEE] rounded-md px-3 py-2 text-sm outline-none focus:border-brand-indigo" />
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1">
              <label className="text-xs font-semibold text-brand-text-muted uppercase">Bank Name</label>
              <input name="bankName" type="text" required placeholder="e.g. Equity Bank" className="w-full border border-[#EEEEEE] rounded-md px-3 py-2 text-sm outline-none focus:border-brand-indigo" />
            </div>
            <div className="space-y-1">
              <label className="text-xs font-semibold text-brand-text-muted uppercase">Account Number</label>
              <input name="accountNo" type="text" required placeholder="e.g. 1234567890" className="w-full border border-[#EEEEEE] rounded-md px-3 py-2 text-sm outline-none focus:border-brand-indigo" />
            </div>
          </div>
        )}
      </div>
      ...
      <div className="flex justify-end gap-3">
        <Link href="/claims" className="px-6 py-2.5 rounded-full border border-[#EEEEEE] text-sm text-brand-text-body hover:bg-[#F8F9FA] transition-colors">
          Cancel
        </Link>
        <button type="submit" disabled={isPending || total === 0}
          className="bg-brand-indigo hover:bg-brand-secondary text-white px-8 py-2.5 rounded-full font-semibold text-sm transition-colors shadow-sm disabled:opacity-50">
