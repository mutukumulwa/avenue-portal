...
export default async function CheckInDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const statusTone = {
    ...
    danger: "border-red-100 bg-red-50 text-brand-error",
    muted: "border-[#EEEEEE] bg-white text-brand-text-body",
  }[status.tone];
  ...
  return (
    <div className="space-y-6">
      {isLive && <AutoRefresh />}
      <div>
        <h1 className="text-2xl font-bold font-heading text-brand-text-heading">Check-In Verification</h1>
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <div className="rounded-lg border border-[#EEEEEE] bg-white p-4">
          <p className="text-xs font-bold uppercase text-brand-text-muted">Facility</p>
          <p className="mt-1 font-bold text-brand-text-heading">{challenge.provider.name}</p>
        </div>
        <div className="rounded-lg border border-[#EEEEEE] bg-white p-4">
          <p className="text-xs font-bold uppercase text-brand-text-muted">Status</p>
          <p className="mt-1 font-bold text-brand-indigo">{challenge.status.replace(/_/g, " ")}</p>
        </div>
        <div className="rounded-lg border border-[#EEEEEE] bg-white p-4">
          <p className="text-xs font-bold uppercase text-brand-text-muted">Expires</p>
          <p className="mt-1 font-bold text-brand-text-heading">{challenge.expiresAt.toLocaleTimeString()}</p>
        </div>
      </section>
          ...
          <div className="flex flex-wrap gap-2">
            {canRestart && (
              <form action={restartCheckInAction}>
                ...
                <button className="rounded-full bg-brand-indigo px-4 py-2 text-xs font-bold text-white hover:bg-brand-secondary">
                  Restart
                </button>
              </form>
            )}
            {canCancel && (
              <form action={cancelCheckInAction} className="flex gap-2">
                <input type="hidden" name="challengeId" value={challenge.id} />
                <input name="reason" placeholder="Cancel reason" className="w-36 rounded-full border border-[#DDDDDD] bg-white px-3 py-2 text-xs text-brand-text-body outline-none" />
                <button className="rounded-full border border-red-200 bg-white px-4 py-2 text-xs font-bold text-brand-error hover:bg-red-50">
                  Cancel
                </button>
              </form>
            )}
      ...
      <section className="rounded-lg border border-[#EEEEEE] bg-white p-5">
        <h2 className="font-bold text-brand-text-heading">Reception Code Match</h2>
        ...
        <form action={confirmVisitCodeAction} className="mt-4 flex flex-col gap-3 sm:flex-row">
          ...
          <input
            ...
            className="w-full rounded-md border border-[#EEEEEE] px-3 py-2 text-sm outline-none focus:border-brand-indigo disabled:bg-brand-bg-alt sm:max-w-48"
          />
          <button disabled={!isAwaitingCode} className="rounded-full bg-brand-indigo px-5 py-2 text-sm font-bold text-white hover:bg-brand-secondary disabled:cursor-not-allowed disabled:opacity-50">
            Confirm match
          </button>
        </form>
      </section>
      ...
      {!challenge.visitVerification && ["PENDING", "FALLBACK_STARTED"].includes(challenge.status) && (
        <section className="grid gap-4 rounded-lg border border-[#EEEEEE] bg-white p-5 md:grid-cols-[1fr_220px]">
          <div>
            <h2 className="font-bold text-brand-text-heading">QR / Manual Pull</h2>
            <p className="mt-1 text-sm text-brand-text-muted">
            ...
            <p className="mt-3 rounded-md bg-brand-bg-alt px-3 py-2 text-xs font-semibold text-brand-text-body">
          ...
          <div className="mt-4 space-y-3">
            {challenge.sharedHealthRecords.map((share: (typeof challenge.sharedHealthRecords)[number]) => (
              <div key={share.id} className="rounded-lg border border-[#EEEEEE] p-4">
                ...
                <p className="mt-3 text-xs text-brand-text-muted">Shared {share.createdAt.toLocaleString()}</p>
              </div>
            ))}
          </div>
        ) : (
      ...
      {!challenge.visitVerification && (
        <section className="rounded-lg border border-amber-200 bg-white p-5">
          <h2 className="font-bold text-brand-text-heading">Photo + Knowledge Fallback</h2>
          <p className="mt-1 text-sm text-brand-text-muted">
          ...
          <form action={knowledgeFallbackAction} className="mt-4 space-y-4">
            ...
            <label className="block">
              <span className="text-xs font-bold uppercase text-brand-text-muted">Photo evidence URL or reference</span>
              <input
                name="photoEvidenceUrl"
                placeholder="Paste uploaded photo reference if captured"
                className="mt-1 w-full rounded-md border border-[#EEEEEE] px-3 py-2 text-sm outline-none focus:border-brand-indigo"
              />
            </label>

            {knowledgePrompts.map((prompt, index) => (
              <label key={prompt.key} className="block">
                <span className="text-xs font-bold uppercase text-brand-text-muted">{prompt.prompt}</span>
      ...
      <section className="rounded-lg border border-[#EEEEEE] bg-white">
        <div className="border-b border-[#EEEEEE] px-5 py-4">
          <h2 className="font-bold text-brand-text-heading">Audit Trail</h2>
        </div>
        <div className="divide-y divide-[#EEEEEE]">
          {challenge.events.map((event) => (
            <div key={event.id} className="px-5 py-3 text-sm">
              ...
              <p className="text-xs text-brand-text-muted">{event.createdAt.toLocaleString()}</p>
            </div>
          ))}
        </div>
      </section>
