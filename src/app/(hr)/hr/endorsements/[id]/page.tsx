  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        <Link href="/hr/endorsements" className="text-avenue-text-muted hover:text-avenue-indigo transition-colors">
          <ArrowLeft size={20} />
        </Link>
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-avenue-text-heading font-heading">{endorsement.endorsementNumber}</h1>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="bg-white border border-[#EEEEEE] rounded-[8px] p-5 shadow-sm space-y-3">
          <h2 className="font-bold text-avenue-text-heading font-heading border-b border-[#EEEEEE] pb-2">Request</h2>
          {[
            ...
            <div key={item.label} className="flex justify-between gap-4 text-sm py-1 border-b border-[#EEEEEE]/50 last:border-0">
              <span className="text-avenue-text-muted">{item.label}</span>
              <span className="font-semibold text-avenue-text-heading text-right">{item.value}</span>
            </div>
          ))}
        </div>

        <div className="bg-white border border-[#EEEEEE] rounded-[8px] p-5 shadow-sm space-y-3">
          <h2 className="font-bold text-avenue-text-heading font-heading border-b border-[#EEEEEE] pb-2">Financial Impact</h2>
          <p className="text-3xl font-bold text-avenue-indigo">
      ...
      <div className="bg-white border border-[#EEEEEE] rounded-[8px] p-5 shadow-sm space-y-3">
        <h2 className="font-bold text-avenue-text-heading font-heading border-b border-[#EEEEEE] pb-2">Change Details</h2>
        {Object.entries(details).filter(([, value]) => value !== null && value !== "").map(([key, value]) => (
          <div key={key} className="flex justify-between gap-4 text-sm py-1.5 border-b border-[#EEEEEE]/50 last:border-0">
            <span className="text-avenue-text-muted">{KEY_LABELS[key] ?? key.replace(/([A-Z])/g, " $1").trim()}</span>
            <span className="font-semibold text-avenue-text-heading text-right">{String(value)}</span>
          </div>
        ))}
      ...
      {endorsement.documents.length > 0 && (
        <div className="bg-white border border-[#EEEEEE] rounded-[8px] p-5 shadow-sm space-y-3">
          <h2 className="font-bold text-avenue-text-heading font-heading border-b border-[#EEEEEE] pb-2">Documents</h2>
          {endorsement.documents.map((document) => (
            <a key={document.id} href={document.fileUrl} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-sm font-semibold text-avenue-indigo hover:underline">
