  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-brand-text-heading font-heading">Utilization Dashboard</h1>
          <p className="text-brand-text-body mt-1">Real-time aggregate benefit usage for {group?.name || "your group"}.</p>
        </div>
      </div>
          ...
          return (
            <div key={s.label} className="bg-white border border-[#EEEEEE] rounded-[8px] p-5 shadow-sm">
              <div className="flex justify-between items-start">
                <p className="text-xs font-bold text-brand-text-muted uppercase tracking-wide">{s.label}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white border border-[#EEEEEE] rounded-[8px] p-6 shadow-sm">
          <h3 className="font-bold text-brand-text-heading mb-4">Spend by Benefit Category</h3>
          <div className="space-y-4">
            {categoryAggRaw.sort((a,b) => Number(b._sum.approvedAmount) - Number(a._sum.approvedAmount)).map(c => (
              <div key={c.benefitCategory}>
                ...
                <p className="text-xs text-brand-text-muted mt-1">{c._count} claims</p>
              </div>
            ))}
            {categoryAggRaw.length === 0 && (
              <p className="text-sm text-brand-text-muted text-center py-6">No localized claims data available yet.</p>
            )}
          </div>
        </div>

        <div className="bg-white border border-[#EEEEEE] rounded-[8px] p-6 shadow-sm flex flex-col justify-center text-center">
          <Activity size={40} className="mx-auto text-brand-border mb-4" />
          <h3 className="font-bold text-brand-text-heading text-lg">Detailed Trending</h3>
