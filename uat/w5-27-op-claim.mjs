...
await shot(p, 'w5-27-auto-adjudication-settings')
...
await shot(p, 'w5-27-peter-pa-hold')
...
console.log(await p2.evaluate(() => {
  ...
  return '→ OUTPATIENT/OUTPATIENT/2026-07-04'
}))
await clickText(p2, 'button', 'Next'); await sleep(1400)
...
console.log(await p2.evaluate(() => {
  ...
  setV(vis('input[placeholder*="Type description" i]').at(-1), 'General practitioner consultation')
...
await shot(p2, 'w5-27-op-claim-submitted')
await b.close()
console.log('DONE')
