...
const db = vi.hoisted(() => {
  ...
  return {
    ...
    clinicalCase: { count: vi.fn(async () => 0), create: vi.fn(async (a: any) => ({ id: "case1", ...a.data })), findFirst: vi.fn(async () => null), findUnique: vi.fn(), update: vi.fn(async (a: any) => a.data) },
    ...
    letterOfUndertaking: { findUnique: vi.fn(), findFirst: vi.fn(async () => null), update: vi.fn(), count: vi.fn(async () => 0), create: vi.fn() },
    claim: { count: vi.fn(async () => 41), create: vi.fn(async (a: any) => ({ id: "clm1", ...a.data })), findFirst: vi.fn(async () => null), findUnique: vi.fn(), findMany: vi.fn(async (): Promise<any[]> => []) },
    activityLog: { create: vi.fn(async () => ({})) },
